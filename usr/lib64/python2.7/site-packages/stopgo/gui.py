...
class GUI(wx.Frame):
    ...
    def InitUI(self,rootdir):
        ...
        self.panel3 = wx.lib.scrolledpanel.ScrolledPanel(self,size=(self.screenWidth,300),style = wx.TAB_TRAVERSAL,name="panel3")
        vbox.Add(self.panel3, 1, wx.EXPAND)

        self.panel3.SetupScrolling(scroll_y = False,scroll_x = True)
    ...
    def BindKeys(self,dbfile):
        ...
        self.Bind(wx.EVT_BUTTON, lambda event, args=(): self.CaptureCanvas(event), self.brec)
        #self.brec.Bind(wx.EVT_BUTTON, self.CaptureCanvas)
    ...
    def NewFile(self,e):
        ...
        sd = wx.FileDialog(self, message='Save file as...', 
            defaultDir=dirname, defaultFile='my_animation',
            wildcard=wcd, 
            style=wx.FD_SAVE|wx.FD_OVERWRITE_PROMPT)

        if sd.ShowModal() == wx.ID_OK:
            projnam = sd.GetFilename()
            projpath= os.path.join(sd.GetPath(),projnam + '.stopgo')
            ...
            os.makedirs( os.path.join(os.path.dirname(projpath),'tmp'))
            if not os.path.exists(os.path.join(projpath,projnam)):
        ...
        self.rendir = os.path.join(os.path.dirname(projpath),'tmp')
    ...
    def BuildTimeline(self,dbfile):
        ...
        if not lsdir:
            ...
            latestfram = int(str(sorted(lsdir)[-1]).split('.')[0])
        
        tbl_timeline = [file for file in sorted(lsdir) if file.endswith('.png')]
    ...
    def OpenFile(self,e,dbfile):

        wcd = 'All files (*)|*|StopGo files (*.db)|*.db'
        openMsg='Choose a project file'

        # what dir to look in when offering to open
        try:
            dbfile = self.clargs['project']
        except OSError:
            dirname = os.path.expanduser('~')
        ...
        # if here then opening from shell
        try: #is this a valid stopgo project file
            ...
            d=["images","thumbs","trash","tmp"]
            for i in d:
                if not os.path.exists(os.path.join( os.path.dirname(dbfile), i)):
                    os.makedirs(os.path.join(os.path.dirname(dbfile),i))

            self.imgdir = os.path.join( os.path.dirname(dbfile), 'images')
            self.thumbdir = os.path.join( os.path.dirname(dbfile), 'thumbs')
            self.trashdir = os.path.join( os.path.dirname(dbfile), 'trash')
            self.rendir = os.path.join( os.path.dirname(dbfile), 'tmp')
    ...
    def CaptureCanvas(self,e):
        """Determine if playing. Take snapshot if in REC mode.
        """
        playstat = str(self.stream.get_state())
        logging.exception(playstat)

        if playstat.endswith('Playing'):
            #take a picture
            self.TakeSnapshot()

        elif playstat.endswith('Stopped') and self.hasSelected:
            #print("We are stopped, but in PLAY mode.")
            ...
            img = self.MakeThumbnail(os.path.join(os.path.dirname(self.rootdir),'share','stopgo','images','s.png'), self.screenWidth/2)#640
            self.PaintCanvas(img)
            ## this triggers OnTimer EVENT
            if self.prefdate == 1:
                self.timer.Start(1000/int(self.myprefs['fps']))
            else:
                self.timer.Start(1000/8)

        elif playstat.endswith('Stopped'):
            print("We are playing, so we are in PLAY mode.")
            print("Stop now and set SELECTED to latest.")
            self.timer.Stop()#stop timer
            
            self.bplay.SetBitmapLabel(self.bplayicon)#enable play button
            self.blick = 0 #send counter back to start. in future, set to latestframe?
            ## TODO use latestframe value as marker, so inserts would be possible.
            self.brec.SetBitmapLabel(self.brecxicon)#disable record btn for now
    ...
    def OnRender(self,e,dbfile):
        ...
        r = os.listdir(self.imgdir)

        for i,f in enumerate(sorted(r)):
            if f.endswith(".png"):
                shutil.copyfile(os.path.join(self.imgdir,f), os.path.join(os.path.dirname(dbfile),self.rendir)+"/"+"render_"+str(i).zfill(3)+".png")
        ...
        if GoRender == 42:
            ...
            if _plat.startswith('linux'):
                P = subprocess.Popen(self.myprefs['encoder'] + " -r "+self.myprefs['fps']+" -pattern_type glob -i '*.png' -b:v "+self.myprefs['bitrate']+" -s " + self.myprefs['profile'] + " -an -pix_fmt yuv420p -deinterlace -y "+sc_out+'.'+self.myprefs['container'], cwd=self.rendir, shell=True)
                ...
                lsdir = os.listdir(self.imgdir)
                stnum  = int(sorted(lsdir))[0].split('.')[0]
                aratio = int(''.join(n for n in self.myprefs['profile'] if n.isdigit()))
                P = subprocess.Popen(self.myprefs['encoder'] +' -f image2 -start_number ' + str(stnum) + ' -r '+self.myprefs['fps']+' -s '+self.myprefs['profile']+' -i \"' + self.imgdir + '\%03d.png\" -b:v '+self.myprefs['bitrate']+' -vf scale='+str(aratio)+':-1 -aspect 16:9 -an -pix_fmt yuv420p -deinterlace -y -threads 2 '+sc_out+'.'+self.myprefs['container'], cwd=self.rendir, shell=True)
    ...
    def OnTimer(self,e):
        ...
        lsdir = os.listdir(self.imgdir)
        self.framlist = [file for file in sorted(lsdir) if file.endswith('.png')]

        if self.timer.IsRunning():
            print(self.timer.IsRunning())
            #try: #this is a lazy hack for-loop
            ...
            self.timer.Stop()
            logging.exception('Timer expired')
            self.bplay.SetBitmapLabel(self.bplayicon)
            ## TODO set to latestfram instead?
            self.blick = 0
            ## go straight back to REC MODE?
            self.stream.play() ## TODO is this what Jess wants? 
            #self.brec.SetBitmapLabel(self.brecicon)
            '''
        self.panel2.Refresh()
