...
class GUI(wx.Frame):
    ...
    def InitUI(self,rootdir):
        ...
        #button states
        self.bplayicon = wx.Image(os.path.join(os.path.dirname(rootdir),'share','stopgo','images','play.png')).ConvertToBitmap()
        self.bstopicon = wx.Image(os.path.join(os.path.dirname(rootdir),'share','stopgo','images','stop.png')).ConvertToBitmap()        
        self.brecicon = wx.Image(os.path.join(os.path.dirname(rootdir),'share','stopgo','images','capture.png')).ConvertToBitmap()
        self.brecxicon = wx.Image(os.path.join(os.path.dirname(rootdir),'share','stopgo','images','capture.png')).ConvertToGreyscale().ConvertToBitmap()
        self.bplayxicon = wx.Image(os.path.join(os.path.dirname(rootdir),'share','stopgo','images','play.png')).ConvertToGreyscale().ConvertToBitmap()
    ...
    def OnLeftRelease(self,e):
        ...
        if not self.hasSelected:
            ...
            imgb = wx.BitmapFromImage(img)
            dc = wx.MemoryDC(imgb)
            staricon = wx.Image(os.path.join(os.path.dirname(rootdir),'share','stopgo','images','select.png') )
    ...
    def TakeSnapshot(self,e,args):
        ...
        if self.camset == 1:
            ...
            vidcap = vlc.libvlc_video_take_snapshot(self.player,0,os.path.join(self.imgdir, iname),0,0)
            self.cur.execute('INSERT INTO Timeline VALUES(Null,?,?)', (iname,0))
            # scale for thumbnail
            ...
            img = self.MakeThumbnail(os.path.join(self.imgdir, iname), self.thumbsize)
